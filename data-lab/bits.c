...
int isLessOrEqual(int x, int y) {
  ...
  int case1 = xsign & sign;
  int case2 = (~((y + (~x + 1)) >> 31)) & ~sign;
  return !!(case1 | case2);
}
//4
/* 
...
int howManyBits(int x) {
  int sign, y, bit16, bit8, bit4, bit2, bit1, bit;
  sign = x >> 31;
  y = x ^ sign;
  bit16 = !!(y >> 16) << 4;
  y = y >> bit16;
...
int floatFloat2Int(unsigned uf) {
  int result;
  int sign = uf >> 31;
  int exp = uf >> 23 & 0xFF; // exclude sign bit
  int frac = uf & 0x7FFFFF; // 23 1bit
  int E = exp - 127;
  ...
  result = 1 << E;
  if (E < 23) {
    result = result | (frac >> (23 - E));
  } else {
    result = result | (frac << (E - 23));
  }
